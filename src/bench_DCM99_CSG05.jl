...
const N_VALUES = 40:5:55
const P_RANGE_AFF = (0.3, 0.6)
const P_RANGE_NEG = (0.85, 0.95)
const NUM_ITER = 5
const TIME_LIM_SECS = 90

const DATA_AFF_PATH = joinpath(dirname(@__DIR__), "data", "compare_DCM99_CSG05_aff.csv")
const DATA_NEG_PATH = joinpath(dirname(@__DIR__), "data", "compare_DCM99_CSG05_neg.csv")
    ...
    for n in N_VALUES
        for k in k_values_aff(n)
            for _ in 1:NUM_ITER
                ...
                partial_dcm = run_benchmark(
                    A, k, DelCorsoManzini(), TIME_LIM_SECS; verbose=true
                )
                push!(results_aff, BenchmarkResult(n, k, psi, p, blb, partial_dcm))

                partial_csg = run_benchmark(
                    A, k, CapraraSalazarGonzalez(), TIME_LIM_SECS; verbose=true
                )
                push!(results_aff, BenchmarkResult(n, k, psi, p, blb, partial_csg))
            end
        end
        ...
        for (k, decider_type) in k_values_and_deciders_neg(n)
            for _ in 1:NUM_ITER
                ...
                partial_dcm99 = run_benchmark(
                    A, k, DelCorsoManzini(), TIME_LIM_SECS; verbose=true
                )
                push!(results_neg, BenchmarkResult(n, k, psi, p, blb, partial_dcm99))

                partial_csg05 = run_benchmark(
                    A, k, CapraraSalazarGonzalez(), TIME_LIM_SECS; verbose=true
                )
                push!(results_neg, BenchmarkResult(n, k, psi, p, blb, partial_csg05))
            end
        end
    end
